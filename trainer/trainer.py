...
class Trainer:
    ...
    def __train_epoch(self, train_set: List[Tuple[torch.tensor, torch.tensor]]) -> float:
        ...
        Return:
            float: Loss related to the current epoch.
        """
        self.model.train()
        epoch_loss = 0
    ...
    def __create_batches(self, train_set: List[Tuple[torch.tensor, torch.tensor]]) -> List[Tuple[torch.tensor, torch.tensor]]:
        ...
        for num_batch in range(num_batches):
            ...
            stop_index = stop_index if len(train_set) > stop_index else len(train_set)
            batch_train_data = train_set[num_batch * self.batch_size : stop_index]
            src, trg = zip(*batch_train_data)
            minibatch_tr.append((torch.cat(src, 0), torch.cat(trg, 0)))
        return minibatch_tr

    def __call__(self, train_set: List[Tuple[torch.tensor, torch.tensor]], epochs: int = 10, callbacks: List[Callable]=[], verbose: bool = True) -> None:
        ...
        for epoch in range(epochs):
            ...
            train_loss = self.__train_epoch(mb_train_set)
            end_time = time.time()
            epoch_mins, epoch_secs = self.__epoch_time(start_time, end_time)
            if verbose:
                print(f'Epoch: {epoch+1:02} | Time: {epoch_mins}m {epoch_secs}s')
                print(f'\tLoss: {math.exp(train_loss):7.3f}')
            for callback in callbacks:
                callback(train_loss, (epoch_mins, epoch_secs))